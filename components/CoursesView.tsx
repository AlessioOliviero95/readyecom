...
export default function CoursesView({ products }: CoursesViewProps) {
  ...
  return (
    <>
      {/* Filter bar */}
      <div className="bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl p-4 mb-8 flex flex-col sm:flex-row gap-3 items-stretch sm:items-center shadow-sm">
        {/* Search */}
        ...
        <select
          ...
          className="px-4 py-2.5 text-sm border border-gray-200 dark:border-slate-600 bg-gray-50 dark:bg-slate-700 text-gray-900 dark:text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500"
        >
          {levels.map((l) => (
        ...
        <span className="text-sm text-gray-500 dark:text-gray-400 whitespace-nowrap self-center">
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20">
          ...
          <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Nessun corso trovato</h3>
          <p className="text-gray-500 dark:text-gray-400">Prova a modificare i filtri o la ricerca.</p>
          <button onClick={() => { setSearch(''); setLevelFilter('all'); setSortBy('default'); }} className="mt-6 px-6 py-2.5 bg-blue-600 text-white font-semibold rounded-xl hover:bg-blue-700 transition-colors text-sm">
            Reimposta filtri
          </button>
        </div>
      ) : (
          ...
          {filtered.map((product) => {
            ...
            return (
              <article key={product.id} className="group bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col">
                {/* Thumbnail */}
                <div className="relative h-44 bg-gradient-to-br from-blue-400 via-purple-400 to-pink-400 flex items-center justify-center overflow-hidden">
                ...
                <div className="p-5 flex flex-col flex-1">
                  ...
                  {/* Name */}
                  <h3 className="font-black text-gray-900 dark:text-white text-lg leading-snug mb-1 line-clamp-2">
                    {product.name}
                  </h3>
                  {product.subtitle && (
                    <p className="text-sm text-gray-500 dark:text-gray-400 mb-3 line-clamp-2">{product.subtitle}</p>
                  )}
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    {product.studentsCount && (
                      <span className="text-xs text-gray-400 ml-auto">{product.studentsCount.toLocaleString('it-IT')} studenti</span>
                    )}
                  </div>
                  ...
                  <div className="mt-auto pt-4 border-t border-gray-100 dark:border-slate-700 flex items-center justify-between gap-3">
                    ...
                    <Link
                      ...
                      className="px-5 py-2.5 bg-gradient-to-r from-blue-600 to-purple-600 text-white text-sm font-bold rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all shadow-sm hover:shadow-md hover:scale-[1.03] active:scale-[0.97] whitespace-nowrap"
                    >
                      Scopri →
                    </Link>
                  </div>
